...
import { createPluginWorkerManager, waitForPluginWorkerStartup } from "./services/plugin-worker-manager.js";
import {
...
export async function startServer(): Promise<StartedServer> {
  ...
  const routineDeliveryWorker = routineService(db as any, { pluginWorkerManager });
  if (config.routineDeliveryWorkerEnabled) {
    void routineDeliveryWorker
    ...
    const routineDeliveryInterval = setInterval(() => {
      void routineDeliveryWorker
        .reconcileRunDeliveries()
        .then((result) => {
          if (result.scanned > 0) {
