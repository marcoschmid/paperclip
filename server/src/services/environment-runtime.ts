...
export function environmentRuntimeService(
  ...
  async function loadPersistedLeaseForActiveOperation(
    ...
    const environment = persisted.environmentId
      ? await environmentsSvc.getById(persisted.environmentId)
      : null;
    if (!environment) {
  ...
  const runtime = {
    ...
    async reconcileEnvironmentLeasesOnStartup(): Promise<{
      ...
      const reconcileDestroyedLease = async (input: {
        ...
        const environment = input.lease.environmentId
          ? await environmentsSvc.getById(input.lease.environmentId)
          : null;
