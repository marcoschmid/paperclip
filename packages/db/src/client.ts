...
export function createDb(url: string, options?: DatabaseClientOptions & CreateDbOptions) {
  const resolved = options ?? databaseClientOptionsFromEnv();
  // Upstream-Treiberoptionen aus Umgebung/Aufruf, danach die Fork-Laufzeitoptionen
  // (idle_in_transaction_session_timeout und application_name).
  const sql = postgres(url, {
    ...postgresJsOptions(resolved),
    ...resolveRuntimePostgresOptions(options ?? {}),
  });
