-- Migration: 0136_phase6_memory_tables
...
-- Reissued at 0136 because stable already owns 0098_project_icon and the live
-- migration journal skipped the earlier local 0098 timestamp collision.

ALTER TABLE "projects" ADD COLUMN IF NOT EXISTS "icon" text;
--> statement-breakpoint
-- Stable removed the legacy Mission-Control executor discriminator from the
-- agents schema. Some upgraded live databases still carry the old NOT NULL
-- column/check/default, so remove it before new agent writes use Stable shape.
